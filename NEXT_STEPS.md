...
All five build phases are done. The hosted deploy (below) is the only remaining v1 work, it's blocked on a manual `supabase login` step, and it's the real blocker for playing on phones — everything so far runs against local Docker only, and a phone can't reach `127.0.0.1:54321`.
